...
export default function AddItemPage() {
  ...
  return (
    <div className="w-10/12 max-w-2xl mx-auto mt-20 bg-[#fdfbc8] p-8 rounded-xl shadow-lg">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <button
          type="submit"
          className="bg-[#CE8946] text-white py-3 rounded-md font-semibold hover:opacity-90 transition mt-4"
        >
          Add Item
        </button>
      </form>
      ...
      {items.length > 0 && (
        <div className="mt-8">
          <h2 className="text-2xl font-bold mb-4 text-[#CE8946]">Preview Added Items</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {items.map((item) => (
              <div key={item.id} className="bg-white p-4 rounded-md shadow-md flex flex-col">
                ...
                <p className="text-yellow-400">{'⭐'.repeat(Math.round(item.rating))} ({item.rating})</p>
              </div>
            ))}
          </div>
        </div>
